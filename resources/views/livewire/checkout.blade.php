        ...
        <div class="grid gap-5 md:gap-20 md:grid-cols-2">
            <div class="p-10">
                ...
                <div
                    ...
                    <label for="af-payment-billing-contact" class="inline-block text-sm font-medium dark:text-gray-800">
                        Billing contact
                    </label>
                ...
                <div
                    ...
                    <label for="af-payment-billing-address"
                        class="inline-block text-sm font-medium dark:text-neutral-900">
                        Billing address
                    </label>
                    ...
                    <div class="mt-2 space-y-3">
                        ...
                        <div>
                                        ...
                                        @foreach ($this->regions as $region)
                                            <li class="p-2 cursor-pointer hover:bg-gray-100">
                                                <label for="region-{{ $region->code }}"
                                                    class="w-full inline-block cusor-pointer">
                            ...
                            @error('data.destination_region_code')
                                <p class="mt-2 text-xs text-red-600" id="hs-validation-name-error-helper">Fill in the
                                    Columns Correctly
                                </p>
                            @enderror
                        </div>
                ...
                <!-- End Section -->
                <label for="af-shipping-method" class="inline-block text-sm font-medium dark:text-neutral-900">
                    Shipping Method
                </label>
                        ...
                        @forelse ($this->shipping_methods as $group_name =>$shipping_method_groups)
                            ...
                            @foreach ($shipping_method_groups as $i => $shipping_method)
                                <label for="shipping_method_{{ $shipping_method->hash }}"
                                    class="flex items-center justify-between w-full gap-2 p-2 text-sm bg-white border border-gray-200 rounded-lg focus:border-blue-500 focus:ring-blue-500 dark:bg-white dark:border-neutral-700 dark:text-neutral-900">
                                    <div class="flex items-center justify-start gap-2">
                                        <input type="radio" wire:key="{{ $shipping_method->hash }}"
                                            ...
                                            class="shrink-0 mt-0.5 border-gray-200 rounded-full text-blue-600 focus:ring-blue-500 checked:border-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-white dark:border-neutral-700 dark:checked:bg-blue-500 dark:checked:border-blue-500 dark:focus:ring-offset-gray-400"
                                            id="shipping_method_{{ $shipping_method->hash }}">
                                        @if ($shipping_method->logo_url)
                                            <img src="{{ $shipping_method->logo_url }}" class="h-5" />
                                        @endif
                                        <span
                                            class="text-sm text-gray-600 ms-3 dark:text-neutral-600">{{ $shipping_method->label }}
                                        </span>
                                    </div>
                ...
                <label for="af-payment-method" class="inline-block mt-5 text-sm font-medium dark:text-neutral-900">
                    Payment Method
                </label>
                <div class="mt-2 space-y-3">
                    <div class="grid space-y-2">
                        @foreach ($this->payment_methods->toCollection() as $key => $payment_method)
                            <label for="payment_method_{{ $payment_method->hash }}"
                                class="flex w-full p-2 text-sm bg-white border border-gray-200 rounded-lg focus:border-blue-500 focus:ring-blue-500 dark:bg-white dark:border-neutral-700 dark:text-neutral-900">
                                ...
                                <span
                                    class="text-sm text-gray-800 ms-3 dark:text-neutral-700">{{ $payment_method->label }}</span>
                            </label>
                        @endforeach
                        @error('data.payment_method_hash')
                            <p class="mt-1 text-xs text-red-600" id="hs-validation-name-error-helper">Fill in the
                                Columns Correctly
                            </p>
                        @enderror
            ...
            <div class="p-10">
                ...
                <div class="grid gap-5">
                    ...
                    <ul class="flex flex-col mt-3">
                        <li
                            class="inline-flex items-center px-4 py-3 -mt-px text-sm text-gray-800 border border-gray-200 gap-x-2 first:rounded-t-lg first:mt-0 last:rounded-b-lg dark:border-neutral-700 dark:text-neutral-800">
